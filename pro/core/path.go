...
// RestartVideoSnapshot restarts the video snapshot server for this path.
// This is called when the snapshot configuration file is updated via the API.
func (pa *path) RestartVideoSnapshot() error {
	if pa.conf.HasStaticSource() {
		if handler, ok := pa.source.(*staticsources.Handler); ok {
			return handler.RestartVideoSnapshot()
		}
	}
	return fmt.Errorf("path does not have a static source")
}
